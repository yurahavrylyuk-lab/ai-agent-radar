...
export function isValidAgentAnalysis(value: unknown): value is AgentAnalysis {
  ...
  return nonEmptyString(analysis.name) &&
    nonEmptyString(analysis.category) && agentAnalysisCategories.includes(analysis.category as AgentAnalysisCategory) &&
    nonEmptyString(analysis.summary) &&
    typeof analysis.relevanceScore === "number" && Number.isInteger(analysis.relevanceScore) && analysis.relevanceScore >= 1 && analysis.relevanceScore <= 10 &&
    nonEmptyString(analysis.whyItMatters) && nonEmptyString(analysis.educationalValue) &&
    stringArray(analysis.projectOpportunities) && stringArray(analysis.technologies) &&
...
export function normalizeDiscoveryUrl(value: string): string {
  ...
  try {
    ...
    throw new DiscoveryHistoryError("Discovery URL must be a valid absolute URL.");
  }
}
